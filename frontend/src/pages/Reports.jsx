  ...
  return (
    <div className="space-y-6 text-slate-100 max-w-7xl mx-auto animate-fade-in pb-12">
      ...
      <div className="glass-card p-6 rounded-2xl glow-indigo flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 no-print">
        ...
        <div className="flex gap-2">
          <button
            ...
            className="py-2.5 px-4 text-xs font-semibold bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-300 border border-emerald-500/20 rounded-xl transition-all shadow-sm font-khmer flex items-center gap-2 cursor-pointer disabled:opacity-50"
          >
      ...
      <div className="glass-card rounded-3xl overflow-hidden border border-white/10 p-6 space-y-6">
        ...
        {/* Detailed Horizontal Comparative Ranking Bars */}
        {selectedMetric !== 'all' ? (
          <div className="space-y-4 pt-2">
            <h3 className="text-xs font-bold text-slate-300 font-khmer uppercase tracking-wider flex items-center justify-between">
              <span>{language === 'kh' ? `ចំណាត់ថ្នាក់ប្រៀបធៀបតាមដេប៉ាតឺម៉ង់ (${currentMetricConfig.titleKh})` : `Department Rankings for ${currentMetricConfig.titleEn}`}</span>
              <span className="text-slate-500 font-mono text-[11px]">Total: {currentMetricConfig.totalCount} {currentMetricConfig.unit}</span>
            </h3>
