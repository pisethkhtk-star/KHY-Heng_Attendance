...
const AttendanceEarlyIn = () => {
  ...
  const fetchLogs = async () => {
    try {
      ...
      console.error('Error loading early check-in logs:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  // Compute early check-in minutes and formatted check details for each log
  const processedLogs = useMemo(() => {
    const result = [];

    const defaultS1Start = companyWorkHour?.shift1Start || '08:00';
    const defaultS2Start = companyWorkHour?.shift2Start || '13:00';
    ...
    logs.forEach(log => {
      ...
      // Early In calculation: checkin is strictly earlier than shift start
      if (c1Min !== null && c1Min < s1StartMin) {
        early1 = s1StartMin - c1Min;
      }
      if (c2Min !== null && c2Min < s2StartMin) {
        early2 = s2StartMin - c2Min;
      }

      const totalEarlyMin = early1 + early2;

      // Filter strictly: Only include records where employee checked in early
      if (totalEarlyMin <= 0) return;

      // Format Check string: Show ONLY the Shift that had Early Check-In
      ...
      if (isShift1Early && !isShift2Early) {
        ...
        // BOTH Shift 1 and Shift 2 are early in
        const in1 = formatTimeOnly(log.checkin1);
        const out1 = formatTimeOnly(log.checkout1);
        const in2 = formatTimeOnly(log.checkin2);
        const out2 = formatTimeOnly(log.checkout2);
  ...
  const handleExportCSV = () => {
    ...
    const title = `Early Check-In Report ${startDisplay} to ${endDisplay}`;
    ...
    groupedByEmployee.forEach((group) => {
        ...
        <table class="report-table" border="1" style="border-collapse:collapse; width:100%; border:1px solid #000000; margin-bottom:20px;">
          <thead>
            <tr style="background-color:#f3f4f6;">
              ...
              <th style="border:1px solid #000000; padding:6px 10px; font-weight:bold; width:110px;">Early In</th>
      ...
      group.records.forEach((rec, idx) => {
        ...
        excelHTML += `
          <tr>
            ...
            <td style="border:1px solid #000000; padding:5px 10px; font-weight:bold; color:#059669;">${rowEarly}</td>
      ...
      excelHTML += `
            <tr>
              ...
              <td style="border:1px solid #000000;"></td>
              <td style="border:1px solid #000000;"></td>
              <td style="border:1px solid #000000; font-weight:bold; padding:6px 10px; color:#059669;">${totalDur}</td>
              <td style="border:1px solid #000000;"></td>
            </tr>
          </tbody>
        </table>
    ...
    link.href = url;
    link.download = `Early_Checkin_Report_${startDate}_to_${endDate}.xls`;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-xl font-bold text-white font-khmer flex items-center gap-2">
            ...
            {language === 'kh' ? 'កំណត់ត្រាបុគ្គលិកមកមុនម៉ោង (Early In)' : 'Early Check-In Records'}
          </h1>
          <p className="text-xs text-slate-400 mt-1 font-khmer">
            {language === 'kh'
              ? 'តាមដាន និងទាញយករបាយការណ៍បុគ្គលិកដែលបាន Check In មុនម៉ោងកំណត់ (Shift 1 & Shift 2)'
              : 'Track and export early arrival attendance logs categorized by employee'}
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="glass-card p-4 rounded-2xl glow-indigo border border-emerald-500/20 bg-slate-950/40">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-[11px] font-bold text-slate-400 uppercase tracking-wider font-khmer">
                {language === 'kh' ? 'ចំនួនដងមកមុនសរុប' : 'Total Early Checks'}
              </p>
        ...
        <div className="glass-card p-4 rounded-2xl glow-indigo border border-emerald-500/20 bg-slate-950/40">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-[11px] font-bold text-slate-400 uppercase tracking-wider font-khmer">
                {language === 'kh' ? 'បុគ្គលិកមកមុន' : 'Early Employees'}
              </p>
        ...
        <div className="glass-card p-4 rounded-2xl glow-indigo border border-emerald-500/20 bg-slate-950/40">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-[11px] font-bold text-slate-400 uppercase tracking-wider font-khmer">
                {language === 'kh' ? 'ម៉ោងមកមុនសរុប' : 'Total Early Time'}
              </p>
              <h3 className="text-2xl font-black text-emerald-400 mt-1 font-mono">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3">
          ...
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-9 pr-3 py-2 border border-white/10 bg-slate-950/60 text-white rounded-xl text-xs focus:border-emerald-500 outline-none placeholder:text-slate-500 font-khmer"
            />
          </div>
          ...
          {user.role !== 'Employee' && (
            <div className="relative">
              <BuildingOfficeIcon className="absolute left-3 top-2.5 h-4 w-4 text-slate-400" />
              <select
                value={filterDept}
                onChange={(e) => setFilterDept(e.target.value)}
                className="w-full pl-9 pr-3 py-2 border border-white/10 bg-slate-950/60 text-white rounded-xl text-xs focus:border-emerald-500 outline-none font-khmer cursor-pointer"
              >
                <option value="">{language === 'kh' ? 'គ្រប់ផ្នែកទាំងអស់' : 'All Departments'}</option>
          ...
          <div className="relative">
            <input
              ...
              className="w-full py-2 px-3 border border-white/10 bg-slate-950/60 text-white rounded-xl text-xs focus:border-emerald-500 outline-none font-mono font-semibold"
            />
          </div>
      ...
      {/* Main Content Area - Grouped Employee Early Check-In Tables */}
      {loading ? (
        ...
        <div className="glass-card p-12 rounded-2xl border border-white/10 text-center">
          ...
          <p className="text-sm font-semibold text-slate-400 font-khmer">
            {language === 'kh'
              ? 'មិនមានទិន្នន័យបុគ្គលិកមកមុនម៉ោងក្នុងចន្លោះកាលបរិច្ឆេទនេះទេ'
              : 'No early check-in records found for this period.'}
          </p>
        </div>
      ) : (
              ...
              <div key={group.staffId} className="space-y-2">
                ...
                <div className="flex items-center justify-between px-1">
                  ...
                  <div className="text-xs font-black text-black font-khmer">
                    {language === 'kh' ? 'ចំនួនមកមុន:' : 'Count Early In:'} <span className="font-black text-black">{group.countEarly}</span>
                  </div>
                </div>

                {/* Styled Table matching the exact style of the late table */}
                      ...
                      <tbody className="divide-y divide-slate-200 font-sans">
                        {group.records.map((rec) => (
                          <tr key={rec.id} className="hover:bg-slate-50 transition-colors">
                            ...
                            {/* EARLY IN: Emerald Bold */}
                            <td className="py-3 px-4 border-r border-slate-200 font-black text-emerald-600 whitespace-nowrap font-mono text-xs">
                      ...
                      <tfoot className="bg-slate-50 border-t-2 border-slate-300 text-xs font-bold">
                        <tr>
                          ...
                          <td className="py-3 px-4 border-r border-slate-300 text-black font-sans whitespace-nowrap">
                            <div className="text-xs font-black text-black">Total: <span className="font-black text-emerald-600 font-mono text-xs">{totalDuration}</span></div>
                            <div className="text-[11px] font-black text-black mt-0.5">Count Early In: <span className="font-black text-black font-mono">{group.countEarly}</span></div>
                          </td>
...
export default AttendanceEarlyIn;
